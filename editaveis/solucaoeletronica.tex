...
Após isso foi calculado, como mostra a Eq. \ref{R_MAX}, a frequência de repetição de pulso \emph{ (Pulse Repetition Frequency - PRF)},  que é o número de ciclos de transmissão.
\begin{equation}\label{R_MAX}
    R_{max} =  \frac{c}{2PRF},
\end{equation}

Onde \emph{c} é a velocidade da luz e $R_{max}$ é a distância máxima de alcance do radar. Com isso temos que o valor da PRF é 1.5MHz.

Já o intervalo de repetição do pulso \emph{(Pulse Repetition Interval - PRI)} é o inverso da PRF que nesse caso é de $0.6\mu s$. Através desse intervalo de tempo podemos determinar o tempo de transmissão do pulso que é dado pela Eq. \ref{duty}. 
\begin{equation}\label{duty}
  d_t = \tau . PRF,
\end{equation}


Onde $d_t$ é o \emph{duty cicle} ou largura do pulso e $\tau$ é o tempo de transmissão do pulso. Considerando que o \emph{duty cicle} é de 50$\%$, ou seja, o pulso será mandado na metade do tempo e na outra metade será recebido, o tempo de transmissão é $0.33\mu s$.

%%%%%%%%%parametrôs de potencia e ruido%%%%%%%%%%%%%%
A Eq. \ref{potencia_recebida}, define a potência a ser recebida, onde $P_t$ é a potência transmitida, $P_r$ a potência recebida, $G$ o ganho da antena, $\lambda$ o comprimento de onda do sinal a ser enviado, $\sigma$ a área de seção transversal média do alvo e $R$ a distância do alvo até a antena.
...
A potência máxima a ser transmitida pelo  \emph{Software Defined Radio} (SDR) é de 6dBm, considerando este valor, a distância de 100 m , o ganho da antena que é de 9 dB, o comprimento de onda para a transmissão do pulso a 915 MHz que corresponde a 0,328 m e uma seção média de 1 $m^2$, através da Eq. \ref{potencia_recebida} podemos estimar quanto de potência deve ser transmitida para garantir uma potência mínima na entrada.


Dividindo-se a Eq. \ref{potencia_recebida} pela potência de ruído térmico, Eq. \ref{potencia_ruido}, sendo este o ruído predominante no projeto, onde $P_n$ é a potência de ruído após receptor, k a constante de Boltzman, $T_e$ a temperatura equivalente de ruído e B a largura de banda, encontra-se a Relação Sinal-Ruído (SNR), e a potência que deve ser transmitida para garantir uma boa SNR a entrada do receptor.

\begin{equation}\label{potencia_ruido}
    P_n = k\, T_e\,B
\end{equation}

A temperatura equivalente de ruído, $T_e$, corresponde a soma da temperatura equivalente de ruído da antena, aqui estimada como temperatura ambiente de 290 K, com a temperatura equivalente de ruído do sistema eletrônico, aqui considerados a relação entre o ruído e temperatura equivalente para obter, no final, uma temperatura equivalente de ruído do sistema de 659,23 K, o que implica, através da Eq. \ref{potencia_ruido}, uma potência de ruído de -170,41 dB, tendo em mãos a potência de ruído e a potência de sinal na entrada a SNR será aproximadamente 24,41 dB .
%citar livros sobre o assunto


Desta maneira, a potência a ser transmitida considerando a SNR calculada é de -0,29 dBm.
